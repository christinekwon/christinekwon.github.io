...
class Frame extends THREE.Group {
	constructor(parent, mobile, material) {
		...
		const left_door = this.create_door(
			door_width,
			yPos * 2,
			[-door_width / 2, 0, 0],
			null
		...
		const right_door = this.create_door(
			door_width,
			yPos * 2,
			[door_width / 2, 0, 0],
			new Lock(parent, mobile, material)
		);
		this.add(right_door);
		...
		this.open_left = new TWEEN.Tween(this.left_door.rotation).to(
		...
		this.open_right = new TWEEN.Tween(this.right_door.rotation).to(
			{ x: 0, y: (3 * Math.PI) / 4, z: 0 },
			speed
		);
		// .easing(TWEEN.Easing.Quadratic.In)
	...
	create_door(width, height, position, lock) {
		...
		// radiustop, bottom, height, radialsegments
		const horiz_outer = new THREE.CylinderGeometry(
			outer_radius,
			outer_radius,
		...
		const horiz_inner = new THREE.CylinderGeometry(
			inner_radius,
			inner_radius,
			width - error,
			segments
		...
		const vertical_outer = new THREE.CylinderGeometry(
			outer_radius,
			outer_radius,
		...
		const vertical_inner = new THREE.CylinderGeometry(
			inner_radius,
			inner_radius,
			height - error,
			segments
		...
		const little = new THREE.CylinderGeometry(
			inner_radius,
			inner_radius,
			little_length,
			segments
		...
		little_vertical.position.set(
			-x + little_positioner,
			y - little_positioner / 2,
			inset
		);
		little_horizontal.position.set(
			-x + little_positioner / 2,
			y - little_positioner,
			inset
		);
